...
// SSN/bank fields along the way. Used by both create and update. A client
// can have more than one policy (a rewrite, an add-on, one cancelled and
// replaced), so each row is its own policy - pass the existing policy's id
// to update it in place, or omit it to create a new one.
export function buildDetailsRow(body, ownerId, contactId, policyId) {
  const row = {
    ...
    underwriting_stage: body.underwritingStage || "applied",
    policy_status: body.policyStatus || "active",
    commission_status: body.commissionStatus || "pending",
    term_conversion_deadline: body.policyProduct === "Term" ? body.termConversionDeadline || null : null,

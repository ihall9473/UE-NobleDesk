...
// One policy's full form (ownership, policy details, pipeline/commission,
// personal details, address, beneficiaries, banking) - a client can have
// more than one of these (a rewrite, an add-on, one cancelled and
// replaced), so the client detail page renders one of these per policy.
export default function PolicyForm({ contactId, policy, defaultOpen, onSaved, onDeleted }) {
  ...
  async function removePolicy() {
    if (!policy?.id) return;
    if (!confirm("Remove this policy? This doesn't remove the client, just this one policy.")) return;
    await fetch(`/api/clients/${contactId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ deletePolicyId: policy.id }),
    });
    onDeleted?.();
  }

  const age = calculateAge(form.dateOfBirth);
  const summary = [policy?.carrier || (policy ? "No carrier set" : "New policy"), policy?.policy_product, policy?.coverage_amount ? `$${policy.coverage_amount} coverage` : null]
    .filter(Boolean)
    .join(" · ");

  return (
    <div className="card">
      <div className="row" style={{ cursor: "pointer", marginBottom: open ? 12 : 0 }} onClick={() => setOpen((o) => !o)}>
        <h3 style={{ marginBottom: 0 }}>{summary}</h3>
        <span style={{ color: "#9a9a9a", fontSize: 13 }}>{open ? "Collapse" : "Expand"}</span>
      </div>
      ...
      {open && (
        <form onSubmit={save}>
          ...
          <h3 style={{ fontSize: 15 }}>Ownership</h3>
          ...
          <h3 style={{ fontSize: 15, marginTop: 16 }}>Policy Details</h3>
          <label className="subtitle" style={{ display: "block", marginBottom: 4 }}>
          ...
          <h3 style={{ fontSize: 15, marginTop: 16 }}>Pipeline & Commission</h3>
          ...
          <h3 style={{ fontSize: 15, marginTop: 16 }}>Personal Details (for this policy)</h3>
          ...
          <input autoComplete="off" placeholder="SSN" value={form.ssn} onChange={(e) => set("ssn", e.target.value)} />

          <h3 style={{ fontSize: 15, marginTop: 16 }}>Address (for this policy)</h3>
          <AddressAutocomplete
          ...
          <h3 style={{ fontSize: 15, marginTop: 16 }}>Beneficiaries</h3>
          <BeneficiaryList
          ...
          <h3 style={{ fontSize: 15, marginTop: 16 }}>Banking (for premium draft)</h3>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <button type="submit" disabled={saving}>{saving ? "Saving..." : "Save This Policy"}</button>
            {policy?.id && (
              <button type="button" onClick={removePolicy} style={{ background: "#dc2626" }}>
                Delete This Policy
              </button>
            )}
          </div>
